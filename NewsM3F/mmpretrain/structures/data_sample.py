...
class DataSample(BaseDataElement):
    ...
    # def set_pred_score(self, value: SCORE_TYPE):
    #     """Set `pred_score` field."""
    #     score = format_score(value)
    #     self.set_field(score, 'pred_score', dtype=torch.Tensor)
        
    #     # 如果存在 self.num_classes，但本次的 score size 不匹配，就覆盖
    #     if hasattr(self, 'num_classes'):
    #         if len(score) != self.num_classes:
    #             # ① 直接覆盖
    #             # print(f"[WARNING] Overwriting data_sample.num_classes from {self.num_classes} to {len(score)}")
    #             self.num_classes = len(score)
    #     else:
