...
# time_data = '2024-08-24'
# task_name = '标题党'  # '标题党' or '表意不规范' or '夸大宣传'
# work_dir = f'work_dirs/quality/InternVL-1B_title_{time_data}'

# qs_cloud = 'nlp-ali'  # 'search01' or 'nlp-ali' or 'nj-larc'
# time_data = '2024-08-24'
# task_name = '夸大宣传'  # '标题党' or '表意不规范' or '夸大宣传'
# tmpl_name = '夸大宣传VLM'
# work_dir = f'work_dirs/quality/InternVL-1B_xuanchuan_{time_data}'


qs_cloud = 'nlp-ali'  # 'search01' or 'nlp-ali' or 'nj-larc'
time_data = '2024-08-28'
task_name = '表意不规范'  # '标题党' or '表意不规范' or '夸大宣传'
tmpl_name = 'biaoyi_VLMV0'
work_dir = f'work_dirs/quality/InternVL-1B_{time_data}_{tmpl_name}'

dataset_name = 'trainval_webdataset'
model_path = f'work_dirs/model_cache/InternVL2-1B'
...
tmpl_name = 'video_VLMV0'
work_dir = f'work_dirs/quality/InternVL-1B_{time_data}_{tmpl_name}_8192'
dataset_path = f'/mnt/{qs_cloud}/dataset/cky_data/videoquality'
dataset_path = dataset_path + f'/{dataset_name}/{time_data}'
...
class DataArguments:
    ...
    train_neg_num: int = field(default=61986)  # 训练集负类别数目
    train_pos_num: int = field(default=1650)  # 训练集正类别数目
    dataset_num_proc: int = field(default=1)
    negative_sample_keep_ratio: float = field(default=0.3)

    max_seq_length: int = field(
        default=8192,  # 4096, 6144, 8192
        metadata={
            "help": "Maximum sequence length. Sequences will be right padded (and possibly truncated)."
        },
    )
    IMG_START_TOKEN: str = field(default='<img>')
    IMG_END_TOKEN: str = field(default='</img>')
    IMG_CONTEXT_TOKEN: str = field(default='<IMG_CONTEXT>')
    num_image_token: int = field(default=256)  # int((image_size // patch_size) ** 2 * (config.downsample_ratio ** 2)) 448//14=32 0.5**2=0.25
    num_max_imgs: int = field(default=8)
    target_size: int = field(default=448)
    to_rgb: bool = field(default=True)
    img_mean: np.ndarray = field(default_factory=lambda: np.array([123.675, 116.28, 103.53]))
    img_std: np.ndarray = field(default_factory=lambda: np.array([58.395, 57.12, 57.375]))
    buffer_size: int = field(default=4000)  # 需要修改
...
class TrainingArguments(transformers.TrainingArguments):
    load_from_weights_file: Optional[str] = field(
        default='work_dirs/quality/InternVL-1B_20240912_video_VLMV0_8192/checkpoint-11500/adapter_model.safetensors',
    )
    ...
    # split_batches: bool = field(default=True)
    ...
    # max_steps: int = field(default=60000)
...
class LoraArguments:
    ...
    modules_to_save: Optional[List[str]] = field(
        default_factory=lambda: ['language_model.score'],
